...
class TestSecurityViolationDetector(unittest.TestCase):
    ...
    def test_privilege_escalation_detection(self):
        ...
        user_id = self.db_service.users.create_user(
    ...
    def test_violation_resolution(self):
        ...
        result = self.detector.resolve_violation(
            "resolve_test", "False positive - resolved"
        )

        self.assertTrue(result)

        # Check it's marked as resolved
        resolved_violation = next(
            v for v in self.detector.violations if v.id == "resolve_test"
        )
        self.assertTrue(resolved_violation.resolved)
        self.assertEqual(
            resolved_violation.resolution_notes, "False positive - resolved"
        )
...
class TestSecurityIntegration(unittest.TestCase):
    ...
    def test_complete_security_workflow(self):
        ...
        resolution_result = self.detector.resolve_violation(
            violation_id, "Investigated - legitimate user"
        )
