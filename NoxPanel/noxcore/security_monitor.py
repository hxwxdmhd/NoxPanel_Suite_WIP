...
class SecurityViolationDetector:
    ...
    def _detect_auth_failures(self) -> List[SecurityViolation]:
        ...
        try:
            recent_time = (
                datetime.now() - timedelta(seconds=self.FAILED_LOGIN_WINDOW)
    ...
    def _detect_suspicious_ips(self) -> List[SecurityViolation]:
        ...
        try:
            recent_time = (datetime.now() - timedelta(hours=1)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            with self.db_service.db.get_connection() as conn:
                cursor = conn.cursor()
    ...
    def _detect_unusual_access_patterns(self) -> List[SecurityViolation]:
        ...
        try:
            ...
            if not (normal_hours["start"] <= current_hour <= normal_hours["end"]):
                recent_time = (datetime.now() - timedelta(minutes=30)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                with self.db_service.db.get_connection() as conn:
                    cursor = conn.cursor()
    ...
    def _record_violation(self, violation: SecurityViolation):
        ...
        try:
            ...
            if violation.severity == "CRITICAL":
                logger.critical(
                    f"CRITICAL SECURITY VIOLATION: {violation.title} - {violation.description}"
                )
            elif violation.severity == "HIGH":
                logger.error(
                    f"HIGH SECURITY VIOLATION: {violation.title} - {violation.description}"
                )
            else:
                logger.warning(
                    f"{violation.severity} SECURITY VIOLATION: {violation.title}"
                )
    ...
    def _generate_recommendations(
        ...
        if auth_violations > 5:
            recommendations.append(
                "Consider implementing additional authentication measures (2FA, IP whitelisting)"
            )

        if data_violations > 3:
            recommendations.append(
                "Review data access policies and implement stricter data loss prevention"
            )

        if not recommendations:
            recommendations.append(
                "Security posture is acceptable, continue monitoring"
            )
