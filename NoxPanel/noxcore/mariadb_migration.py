...
class MariaDBConnectionPool:
    ...
    def __init__(
        ...
        if not MARIADB_AVAILABLE:
            raise ImportError(
                "PyMySQL is required for MariaDB support. Install with: pip install pymysql"
            )
...
class DatabaseMigrator:
    ...
    def connect_mariadb(self) -> MariaDBDatabase:
        ...
        if not MARIADB_AVAILABLE:
            raise ImportError(
                "PyMySQL is required for MariaDB migration. Install with: pip install pymysql"
            )
        ...
        with self.sqlite_db.get_connection() as sqlite_conn:
            ...
            while offset < total_records:
                ...
                if offset % (batch_size * 10) == 0:
                    logger.info(
                        f"Migrated {offset}/{total_records} records from {table_name}"
                    )

        return total_count

    def _insert_batch_mariadb(
        ...
        for table in tables:
            try:
                ...
                if sqlite_count != mariadb_count:
                    ...
                    logger.warning(
                        f"Count mismatch in {table}: SQLite={sqlite_count}, MariaDB={mariadb_count}"
                    )

            except Exception as e:
                logger.error(f"Validation error for table {table}: {e}")
                validation_results["validation_passed"] = False
                validation_results["table_comparisons"][table] = {"error": str(e)}
